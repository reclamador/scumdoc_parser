...
class DNIScumParser(BaseScumDocParser):
    ...
    def __init__(self, text):
        ...
        for keyword in self.KEYWORDS:
            searches.append(FuzzySearch(keyword, keyword, 0.7, group='keywords', post_process=FuzzySearch.process_only_true))
        for key, value in self.OCR.items():
            searches.append(RegexSearch(key, regex=value, group="ocr", pre_process=self.pre_process_ocr, post_process=self.post_process_ocr))
